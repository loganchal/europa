...
def run_simulation(
    ...
    *,
    ...
    Parameters mirror the original script with gravity, albedo and
    solar radiation held constant internally. If ``callback`` is provided it
    will be invoked each time step with the current temperature profile,
    the depth array, the ice extent, the current step index and the total
    number of steps. The function returns the final temperature profile,
    depth points and highest ice extent.
    """
    # Physical constants
    ...
    g = 1.315  # Surface gravity (m/s^2)
    albedo = 0.64  # Fraction of solar radiation reflected
    solar_radiation_flux = 50  # Solar flux at the surface (W/m^2)
    ...
    for t in range(1, total_time_steps):
        ...
        if callback is not None:
            callback(temperature.copy(), depth_points, highest_ice_extent, t, total_time_steps)
...
def plot_profile(temperature, depth_points, highest_ice_extent, *, show=True):
    """Create a plot of the water column and return the figure."""
    fig, ax = plt.subplots()
    ax.plot(temperature, depth_points, label="Water Temperature", color="tab:blue")
    ax.set_xlabel("Temperature (°C)")
    ax.set_ylabel("Depth (m)")
    ax.set_title("Water Column")
    ax.axhline(y=highest_ice_extent, color="tab:green", linestyle="--", label="Ice Depth")
    ax.text(temperature[0], highest_ice_extent, f"Ice Depth: {highest_ice_extent:.0f} m", va="bottom")
    ax.invert_yaxis()
    ax.legend()
    ax.grid(True)
    if show:
...
if __name__ == "__main__":
    ...
    plot_profile(temp, depth_pts, ice_extent, show=True)
