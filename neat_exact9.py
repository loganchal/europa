...
def main():
    ...
    for ordinal,U in enumerate(polytopes(path),1):
        ...
        if selected%5000==0:print(json.dumps({'facets':a.facets,'shard':a.shard,'selected':selected,'nonneat':len(nonneat),'unresolved':len(unresolved),'errors':len(errors),'best_inter':best_inter[0],'sec':round(time.time()-t,1)}),flush=True)
